...
export default function ModelLibrary({ref,selected,onExplore,onOpenChange}){
  ...
  useEffect(()=>{
    ...
    Promise.all(['/landmarks/catalog.json','/map/manifest.json'].map(url=>fetch(url,{signal:abort.signal}).then(response=>{if(!response.ok)throw Error('The collection could not load.');return response.json();}))).then(([catalog,manifest])=>{
      const entries=LANDMARKS.map(place=>catalog.find(item=>item.id===place.id)||{id:place.id,name:place.name,bytes:manifest.assets.tower.bytes,interiors:false});
      setModels(entries);
    }).catch(failure=>{if(failure.name!=='AbortError')setError(failure.message);});
    return()=>abort.abort();
  },[open,models,attempt]);
  const model=models?.find(item=>item.id===activeId),place=LANDMARKS.find(item=>item.id===activeId);
      ...
      {open&&(error?<div className="library-error" role="alert"><p>{error}</p><button className="studio-primary" onClick={()=>setAttempt(n=>n+1)}>Retry collection</button></div>:!models?<p className="library-error" role="status">Opening the collection…</p>:<div className="library-body">
        ...
        <div className="library-workbench" ref={scroll}>
          ...
          <aside className="studio-inspector">
            <header className="studio-model-heading"><span className="studio-eyebrow">{LANDMARK_STORIES[activeId].category}</span><h2>{model.name}</h2><p lang="si">{place.sinhala}</p><p lang="ta">{LANDMARK_STORIES[activeId].tamil}</p><small><MapPin/>{coordinateLabel(place.coordinates)}</small></header>
