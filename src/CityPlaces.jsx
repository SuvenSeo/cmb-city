...
export default function CityPlaces({ready,selected,onSelect,showTags,onShowTags,compact}){
  ...
  return <div className="places-control" ref={root} onKeyDown={event=>{
    ...
    <button className="places-trigger" ref={trigger} disabled={!ready} aria-expanded={open} aria-controls="places-panel" onClick={()=>setOpen(!open)}><MapPin/>Explore places<span>{ALL_LANDMARKS.length}</span></button>
    {open&&<div className="places-panel" id="places-panel">
      ...
      <div className="places-list">{ALL_LANDMARKS.map((place,index)=><button key={place.id} className={selected===place.id?'is-selected':''} onClick={()=>choose(place.id)} aria-label={`Fly to ${place.name}`}>
